    ...
    <div class="caja_trasera">
        <div class="caja_trasera_login">
            ...
            <p>Iniciar sesión para ingresar a la página</p>
             <button id="btn_iniciar_secion_vista" onclick="window.location.href='index.php?action=login'">Iniciar sesión</button>
             <br><br>
             <a href="index.php?action=login">O ir a Iniciar Sesión</a>
        </div>
        <div class="caja_trasera_register">
             <h3>Aún no tienes una cuenta</h3>
             <p>Regístrate para que puedas iniciar sesión</p>
             <!-- Este botón es para la animación JS, se puede mantener o simplificar si no se usa más -->
             <button id="btn_Register_vista" onclick="void(0)">Regístrarse</button> <!-- Deshabilitado o que no haga nada si ya estamos en registro -->
        </div>
    </div>

    <div class="contenedor_login_register" style="left: 410px;"> <!-- Ajustado para que el panel de registro sea el activo -->

        <!-- FORMULARIO DE LOGIN (oculto) -->
        <form action="index.php?action=login" method="post" class="formulario_login" id="formulario_login_id" style="display:none;">
            ...
            <br><br>
            <a href="restaurarPasword.php">Olvidé mi Password</a>
        </form>

        <!-- FORMULARIO DE REGISTRO (visible) -->
        <form action="index.php?action=register" method="post" enctype="multipart/form-data" class="formulario_register" id="formulario_register_id">
            <h2>Regístrarse</h2>
            ...
            <?php if (isset($success) && $success): ?>
                ...
                <p><a href="index.php?action=login">Haz clic aquí para iniciar sesión</a></p>
            <?php endif; ?>
            ...
            <select name='tipo' id="tipo" required>
                <option value="" disabled <?php echo !isset($_POST['tipo']) ? 'selected' : ''; ?>>Selecciona tipo de usuario</option>
            ...
            <select name='grado' id="grado" required>
                <option value="" disabled <?php echo !isset($_POST['grado']) ? 'selected' : ''; ?>>Selecciona grado (si aplica)</option>
            ...
            <label>Si eres Docente, selecciona "No aplica".</label>
            <br><br>
...
<!-- El script de animación original (js/script.js) es complejo para replicar aquí sin conocer su estado exacto.
     Por simplicidad, esta vista de registro solo muestra el formulario de registro.
     Si la animación es crucial, el script original necesitaría ser adaptado o incluido.
     Para este ejemplo, he simplificado los botones de las cajas traseras. -->
